...
// 実行前に確定済みExport計画のローカルID、チェックサム、ページング値を置換すること。
...
if (
  startIndex > injectedVariableIds.length
) {
  throw new Error(
    `startIndex exceeds the local Export plan: ${startIndex} > ` +
...
const resolvedRecordsById = new Map();
const resolveRecord = async (variableId) => {
  const cached = resolvedRecordsById.get(variableId);
  ...
  const record = { variable, collection };
  resolvedRecordsById.set(variableId, record);
  return record;
};
...
const buildResult = (batchRecords) => {
  ...
  for (const { variable } of batchRecords) {
  ...
  const batchCollections = new Map();
  for (const record of batchRecords) {
    batchCollections.set(record.collection.id, record.collection);
  }
  ...
  const payload = {
    ...
    collections: [...batchCollections.values()].map((collection) => ({
      id: collection.id,
      key: collection.key,
...
for (let index = startIndex; index < endIndex; index += 1) {
  ...
  const { variable, collection } = await resolveRecord(variableId);
  if (variable.remote) {
    throw new Error(`Remote Variable found in local Export plan: ${variableId}`);
  }
  const candidate = [...records, { variable, collection }];
  const candidateResult = buildResult(candidate);
  if (utf8ByteLength(JSON.stringify(candidateResult)) > maxPayloadBytes) {
    if (records.length === 0) {
      throw new Error(
        `A single local Variable exceeds maxPayloadBytes: ${variableId}`
      );
    }
    break;
  }
  records.push({ variable, collection });
}

const result = buildResult(records);
if (utf8ByteLength(JSON.stringify(result)) > maxPayloadBytes) {
  throw new Error("Local Export payload exceeds maxPayloadBytes");
}
return result;
